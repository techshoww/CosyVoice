...
token_embedding = torch.ones([1,speech_token_len+prompt_token_len, 80], dtype=torch.float32)                          
...
if not finalize:
    flow.forward = flow.export_flow_encoder
    onnx_output = f"flow_encoder_{speech_token_len}.onnx"
else:
    flow.forward = flow.export_flow_encoder_final
    onnx_output = f"flow_encoder_{speech_token_len}_final.onnx"
